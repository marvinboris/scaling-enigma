...
class Attention extends Component {
    ...
    render() {
        ...
        requestsData = requests.map(request => {
            ...
            const editContent = <Edit request={updateObject(request, { page_status: 'attention' })} />;
        ...
        content = (
            <>
                <Row>
                    <List loading={loading} array={requestsData} data={JSON.stringify(requests)} get={this.props.onGetAttentionRequests} bordered add="File a Request" link="/user/requests/add" icon={faCalendarAlt} title="Attention Requests" className="bg-white shadow-sm"
                        fields={[
        ...
        return (
            <>
                <div className="bg-white py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb main="Attention Requests" icon={faCalendarAlt} />
                    <SpecialTitle user icon={faCalendarAlt}>User panel</SpecialTitle>
                    <Subtitle user>Attention Requests</Subtitle>
                </div>
...
const mapDispatchToProps = dispatch => ({
    onGetAttentionRequests: (page, show, search) => dispatch(actions.getAttentionRequests(page, show, search)),
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(Attention));