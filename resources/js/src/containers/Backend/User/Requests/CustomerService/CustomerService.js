...
import { faTachometerAlt, faEnvelope, faTicketAlt, faTasks, faArrowsAlt, faTimes, faEye, faEdit, faTrash, faClock, faLandmark, faCalendarAlt, faDownload, faSpinner, faTimesCircle, faCheckCircle, faFileArchive, faFilePdf, faFileImage, faUser, faBook, faCheck, faExternalLinkAlt } from '@fortawesome/free-solid-svg-icons';
...
class CustomerService extends Component {
    ...
    render() {
        ...
        requestsData = requests.map(request => {
            ...
            const editContent = <Edit request={updateObject(request, { page_status: 'customer-service' })} />;
        ...
        content = (
            <>
                <Row>
                    <List loading={loading} array={requestsData} data={JSON.stringify(requests)} get={this.props.get} total={total} bordered add="File a Request" link="/user/requests/add" icon={faCalendarAlt} title="Customer Service Requests" className="bg-white shadow-sm"
                        fields={[
        ...
        return (
            <>
                <div className="bg-white py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb main="Customer Service Requests" icon={faCalendarAlt} />
                    <SpecialTitle user icon={faCalendarAlt}>User panel</SpecialTitle>
                    <Subtitle user>Customer Service Requests</Subtitle>
                </div>
...
const mapDispatchToProps = dispatch => ({
    get: (page, show, search) => dispatch(actions.getCustomerServiceRequests(page, show, search)),
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(CustomerService));