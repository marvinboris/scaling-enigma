...
class Limarket extends Component {
    state = {
        countries: []
    }
    ...
    render() {
        ...
        requestsData = requests.map(request => {
            ...
            const editContent = <Edit request={updateObject(request, { page_status: 'limarket' })} />;
        ...
        content = (
            <>
                <Row>
                    <List loading={loading} array={requestsData} data={JSON.stringify(requests)} get={this.props.get} total={total} bordered add="File a Request" link="/user/requests/add" icon={faCalendarAlt} title="Limarket Requests" className="bg-white shadow-sm"
                        fields={[
        ...
        return (
            <>
                <div className="bg-white py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb main="Limarket Requests" icon={faCalendarAlt} />
                    <SpecialTitle user icon={faCalendarAlt}>User panel</SpecialTitle>
                    <Subtitle user>Limarket Requests</Subtitle>
                </div>
...
const mapDispatchToProps = dispatch => ({
    get: (page, show, search) => dispatch(actions.getLimarketRequests(page, show, search)),
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(Limarket));