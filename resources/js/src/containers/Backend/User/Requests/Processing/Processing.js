...
class Processing extends Component {
    ...
    render() {
        ...
        requestsData = requests.map(request => {
            ...
            const editContent = <Edit request={updateObject(request, { page_status: 'processing' })} />;
        ...
        content = (
            <>
                <Row>
                    <List loading={loading} array={requestsData} data={JSON.stringify(requests)} get={this.props.onGetProcessingRequests} total={total} bordered add="File a Request" link="/user/requests/add" icon={faCalendarAlt} title="Processing Requests" className="bg-white shadow-sm"
                        fields={[
        ...
        return (
            <>
                <div className="bg-white py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb main="Processing Requests" icon={faCalendarAlt} />
                    <SpecialTitle user icon={faCalendarAlt}>User panel</SpecialTitle>
                    <Subtitle user>Processing Requests</Subtitle>
                </div>
