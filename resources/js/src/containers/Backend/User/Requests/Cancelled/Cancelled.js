        ...
        if (loading) content = <Col xs={12}>
            ...
            if (requests) {
                ...
                const requestsData = requests.map(request => {
                    ...
                    const editContent = <Edit request={updateObject(request, { page_status: 'cancelled' })} />;
                ...
                content = (
                    <>
                        <Row>
                            <List array={requestsData} data={JSON.stringify(requests)} bordered add="File a Request" link="/user/requests/add" icon={faCalendarAlt} title="Cancelled Requests" className="bg-white shadow-sm"
                                fields={[
        ...
        return (
            <>
                <div className="bg-white py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb main="Cancelled Requests" icon={faCalendarAlt} />
                    <SpecialTitle user icon={faCalendarAlt}>User panel</SpecialTitle>
                    <Subtitle user>Cancelled Requests</Subtitle>
                </div>
...
const mapDispatchToProps = dispatch => ({
    onGetCancelledRequests: () => dispatch(actions.getCancelledRequests()),
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(Cancelled));