...
class Dev extends Component {
    ...
    render() {
        ...
        content = (
            <>
                <Row>
                    <List loading={loading} array={requestsData} data={JSON.stringify(requests)} get={this.props.onGetDevRequests} total={total} bordered add="File a Request" link="/user/requests/add" icon={faCalendarAlt} title="Dev Requests" className="bg-white shadow-sm"
                        fields={[
        ...
        return (
            <>
                <div className="bg-white py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb main="Dev Requests" icon={faCalendarAlt} />
                    <SpecialTitle user icon={faCalendarAlt}>User panel</SpecialTitle>
                    <Subtitle user>Dev Requests</Subtitle>
                </div>
...
const mapDispatchToProps = dispatch => ({
    onGetDevRequests: (page, show, search) => dispatch(actions.getDevRequests(page, show, search)),
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(Dev));