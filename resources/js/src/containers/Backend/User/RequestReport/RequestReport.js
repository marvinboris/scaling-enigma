...
import Description from '../Requests/Description';
...
class RequestReport extends Component {
    ...
    render() {
        ...
        requestsData = requests.map(request => {
            ...
            const editContent = <Edit request={updateObject(request, { page_status: 'report' })} />;
            ...
            <Row>
                <List loading={loading} array={requestsData} data={JSON.stringify(requests)} get={this.props.onGetRequests} total={total} bordered add="File a Request" link="/user/requests/add" icon={faCalendarAlt} title="Request Report" className="bg-white shadow-sm"
                    fields={[
                        ...
                        { name: 'Admin E-Mail', key: 'edited_by' },
                        { name: 'Admin IP', key: 'user_ip' },
                        { name: 'Admin Browser', key: 'user_browser' },
        ...
        return (
            <>
                <div className="bg-white py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb main="Request Report" icon={faCalendarAlt} />
                    <SpecialTitle user icon={faCalendarAlt}>User panel</SpecialTitle>
                    <Subtitle user>Request Report</Subtitle>
                </div>
...
const mapDispatchToProps = dispatch => ({
    onGetRequests: (page, show, search) => dispatch(actions.getRequests(page, show, search)),
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(RequestReport));